package org.optaplanner.examples.cloudbalancing.constaints;
...
public class CloudBalancingConstraintProvider implements ConstraintProvider {
	...
	public Constraint[] defineConstraints(ConstraintFactory constraintFactory) {
		return new Constraint[]{
				...
				atMost4ProcessesPerComputer(constraintFactory)
		};
	}
	...
	private Constraint atMost4ProcessesPerComputer(ConstraintFactory constraintFactory) {
		return constraintFactory.from(CloudProcess.class)
				.groupBy(CloudProcess::getComputer, count())
				.filter((computer, count) -> count > 4)
				.penalize("atMost4ProcessesPerComputer",
						HardSoftScore.ONE_HARD,
						(computer, count) -> count - 4);
	}
